...
pub const ProtoSlot = struct {
    ...
    pub fn initConstant(name: []const u8, parent: SlotProperties.ParentFlag) ProtoSlot {
        return init(name, parent, .Constant, .NotArgument, .NotInherited, null);
    }

    pub fn initAssignable(name: []const u8, parent: SlotProperties.ParentFlag) ProtoSlot {
        return init(name, parent, .Assignable, .NotArgument, .NotInherited, null);
    }

    pub fn initArgument(name: []const u8) ProtoSlot {
        return init(name, .NotParent, .Assignable, .Argument, .NotInherited, null);
    }

    pub fn initInherited(name: []const u8, value: Value) ProtoSlot {
    ...
    pub fn fromExistingSlot(slot: Slot) ProtoSlot {
        const name = slot.name.asByteArray().getValues();
        if (slot.isInherited())
            return initInherited(name, slot.value);
        if (slot.isArgument())
            return initArgument(name);
        if (slot.isAssignable())
            return initAssignable(name, if (slot.isParent()) .Parent else .NotParent);
        return initConstant(name, if (slot.isParent()) .Parent else .NotParent);
    }
    ...
    fn init(
        ...
        const properties = SlotProperties.init(name, parent, assignable, argument, inherited, .NotInheritedChild);
        return .{ .name = name, .properties = properties, .value = value };
    }

    /// Return whether a slot with my slot name exists in the part of the object before me.
    fn slotWithMyNameExistsBeforeMe(self: ProtoSlot, ast_slots: []const AST.SlotNode, inherited_slot_values: []const Heap.Tracked) bool {
        var inherited_slot_values_offset = @intCast(isize, inherited_slot_values.len) - 1;
        // NOTE: Walking backwards to find the first slot that overwrote the
        //       earlier ones.
        for (ast_slots) |_, index| {
            const slot_node = ast_slots[ast_slots.len - 1 - index];

            if (slot_node.is_inherited) {
                const inherited_slots = inherited_slot_values[@intCast(usize, inherited_slot_values_offset)].getValue().asObject().asSlotsObject().getSlots();
                for (inherited_slots) |_, inherited_index| {
                    const inherited_slot = inherited_slots[inherited_slots.len - 1 - inherited_index];

                    if (std.mem.eql(u8, self.name, inherited_slot.name.asByteArray().getValues()))
                        return true;
                }

                inherited_slot_values_offset -= 1;
            }

            if (std.mem.eql(u8, self.name, slot_node.name)) {
    ...
    /// executed.
    pub fn requiredSlotSpace(self: ProtoSlot, ast_slots: []const AST.SlotNode, inherited_slot_values: []const Heap.Tracked) u32 {
        if (self.properties.isInherited()) {
            // FIXME: Turn this into a runtime error instead of a panic.
            if (self.slotWithMyNameExistsBeforeMe(ast_slots, inherited_slot_values))
                @panic("Name collision is not allowed for inherited slots");

            // 1 for the inherited slot itself.
            ...
            next_slot: for (self.value.?.asObject().asSlotsObject().getSlots()) |inherited_slot| {
                ...
                var proto_inherited_slot = ProtoSlot.fromExistingSlot(inherited_slot);
                if (proto_inherited_slot.slotWithMyNameExistsBeforeMe(ast_slots, inherited_slot_values))
                    continue :next_slot;
        ...
        return if (self.slotWithMyNameExistsBeforeMe(ast_slots, inherited_slot_values)) 0 else 1;
    }
};
