...
pub fn StringPrint(allocator: Allocator, heap: *Heap, message_range: Range, tracked_receiver: Heap.Tracked, arguments: []Heap.Tracked, context: *InterpreterContext) !Completion {
    _ = heap;
    _ = arguments;
    _ = message_range;
    _ = context;
    ...
    switch (receiver.getType()) {
        ...
        .ObjectReference => {
            if (!(receiver.asObject().isByteVectorObject())) {
                return Completion.initRuntimeError(allocator, "Expected ByteVector as the receiver of _StringPrint", .{});
            }
...
pub fn ByteVectorSize(allocator: Allocator, heap: *Heap, message_range: Range, tracked_receiver: Heap.Tracked, arguments: []Heap.Tracked, context: *InterpreterContext) !Completion {
    _ = heap;
    _ = arguments;
    _ = message_range;
    _ = context;
    ...
    if (!(receiver.isObjectReference() and receiver.asObject().isByteVectorObject())) {
        return Completion.initRuntimeError(allocator, "Expected ByteVector as _ByteVectorSize receiver", .{});
    }
...
/// byte vector.
pub fn ByteAt(allocator: Allocator, heap: *Heap, message_range: Range, tracked_receiver: Heap.Tracked, arguments: []Heap.Tracked, context: *InterpreterContext) !Completion {
    _ = heap;
    _ = message_range;
    _ = context;
    ...
    if (!(receiver.isObjectReference() and receiver.asObject().isByteVectorObject())) {
        return Completion.initRuntimeError(allocator, "Expected ByteVector as _ByteAt: receiver", .{});
    }

    if (!argument.isInteger()) {
        return Completion.initRuntimeError(allocator, "Expected integer as _ByteAt: argument", .{});
    }
...
pub fn ByteAt_Put(allocator: Allocator, heap: *Heap, message_range: Range, tracked_receiver: Heap.Tracked, arguments: []Heap.Tracked, context: *InterpreterContext) !Completion {
    _ = heap;
    _ = message_range;
    _ = context;
    ...
    if (!(receiver.isObjectReference() and receiver.asObject().isByteVectorObject())) {
        return Completion.initRuntimeError(allocator, "Expected ByteVector as _ByteAt:Put: receiver", .{});
    }

    if (!first_argument.isInteger()) {
        return Completion.initRuntimeError(allocator, "Expected integer as first _ByteAt:Put: argument", .{});
    }
    if (!second_argument.isInteger()) {
        return Completion.initRuntimeError(allocator, "Expected integer as second _ByteAt:Put: argument", .{});
    }
    ...
    if (new_value < 0 or new_value > 255) {
        return Completion.initRuntimeError(allocator, "New value passed to _ByteAt:Put: cannot be cast to a byte", .{});
    }
...
/// receiver's size.
pub fn ByteVectorCopySize(allocator: Allocator, heap: *Heap, message_range: Range, tracked_receiver: Heap.Tracked, arguments: []Heap.Tracked, context: *InterpreterContext) !Completion {
    _ = message_range;
    _ = context;
    ...
    if (!(receiver.isObjectReference() and receiver.asObject().isByteVectorObject())) {
        return Completion.initRuntimeError(allocator, "Expected ByteVector as _ByteVectorCopySize: receiver", .{});
    }

    if (!argument.isInteger()) {
        return Completion.initRuntimeError(allocator, "Expected Integer as _ByteVectorCopySize: argument", .{});
    }

    var values = receiver.asObject().asByteVectorObject().getValues();
    const size = argument.asInteger();
    if (size >= values.len) {
        return Completion.initRuntimeError(allocator, "_ByteVectorCopySize: argument exceeds receiver's size", .{});
    }

    if (size < 0) {
        return Completion.initRuntimeError(allocator, "_ByteVectorCopySize: argument must be positive", .{});
    }
